...
/*
Специальные эффекты оружия и брони
Стрелковое
  - в первый ход противник не отвечает
Щит
  - шанс блока в ближнем бою
  - большой шанс блока против стрел, болтов и т.д.
Осадный щит
  - Очень высокий шанс блока против стрел
Одноручное копье
  - первый удар всегда без ответа
Двуручное копье
  - если у конницы шанс убить опонента с одного удара
  - повышенный урон против пехоты
Топор
  - повышенный урон по щитам
  - шанс сломать щит при выставленном блоке
Двуручный топор
  - повышенный урон по щитам
  - повышенный шанс сломать щит при выставленном блоке
Арбалет
  - шанс игнорировать броню


Типы войск
1 - легкая пехота
  - кожанная броня, либо одежда
  - одноручное оружие ближнего боя
  - легкие луки и арбалеты
2 - тяжелая пехота
  - кольчуга, пластинчатые или латы в качестве брони
  - наличие щита, опциональное
3 - легкая конница
  - кожанная или кольчуга
  - копья, длинные мечи, булавы
  - легкие луки
  -
4 - тяжелая конница
  - пластинчатые или латные доспехи
  - чаще всего щит
  - тяжелые длинные копья
  - длинные мечи, булавы

Бонус к урону и защите по типу войск

1 - легкая пехота
2 - тяжелая пехота с щитом
3 - двуручные копейщики
4 - копейщики с щитом
5 - лучники
6 - арбалетчики
7 - тяжелые арбалетчики со стационарным щитом
8 - легкая конница
9 - тяжелая конница
...
/*
Механика удара в бою
1 - считаем попал или нет
  - промах, уворот
2 - если попал и есть щит, то считаем блок щитом
3 - если блок щитом удачный, то считаем пробитие и урон для шита
4 - считаем пробитие и урон
...
Предусловия:
  - шанс пробития не может быть больше 0,95
  - всегда есть минимальный урон
  - если удар попал по блоку щитом, то урон и пробитие считается относительно щита
  - МодификаторОружия - модификатор шанса пробития конкретным типом оружия конкретного типа брони
Если удар достиг цели, то:
  1 - производится расчет шанса пробития брони
  ШансПробития = БазовоеЗначение * МодификаторОружия + БонусНавыковКомандира + БонусНавыковСолдата
  Если ШансПробития > 95 то ШансПробития = 95
  2 - Получаем CлучайноеЧисло 1-100
  3 - если ШансПробития >= СлучайноеЧисло то Пробил, полученный урон считается без учета показателя брони
  4 - если ШансПробития < СлучайноеЧисло то НеПробил, от полученного урона вычитается показатель брони
*/
